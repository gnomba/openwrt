#!/bin/sh
### uci-defaults Livinet Li320 [ROUTERICH mod] ###
...
rr_mod() {
    ...
    echo 'RR-3.12.1' > /etc/routerich_release
...
add_repository() {
    ...
    echo "src/gz routerich_core https://packages.routerich.ru/24.10/mediatek/filogic/24.10.8/core
src/gz routerich_base https://packages.routerich.ru/24.10/mediatek/filogic/24.10.8/base
src/gz routerich_luci https://packages.routerich.ru/24.10/mediatek/filogic/24.10.8/luci
src/gz routerich_packages https://packages.routerich.ru/24.10/mediatek/filogic/24.10.8/packages
src/gz routerich_routing https://packages.routerich.ru/24.10/mediatek/filogic/24.10.8/routing
src/gz routerich_telephony https://packages.routerich.ru/24.10/mediatek/filogic/24.10.8/telephony
...
set_wifi() {
	...
	last_4_mac_wan="$(echo "$MAC" | awk -F: '{print toupper($5$6)}')"
	echo "last_4_mac_wan=$last_4_mac_wan"
	wlan_name_2g="Livinet_${last_4_mac_wan}"
...
set_system() {
	...
	last_4_mac_wan="$(echo "$MAC" | awk -F: '{print toupper($5$6)}')"
	echo "last_4_mac_wan=$last_4_mac_wan"
	hostname="Livinet_${last_4_mac_wan}"
...
set_ksmbd() {
	...
	uci -q set ksmbd.@globals[0].description='Livinet Share'
	uci commit ksmbd
